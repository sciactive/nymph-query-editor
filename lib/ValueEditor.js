(function (global, factory) {
	...
	function create_if_block(state, component) {
		...
		function select_change_handler() {
			...
			component.set({ valueTypeCurrent: selectValue(select) });
			select_updating = false;
		}
		...
		return {
			...
			h: function hydrate() {
				...
				if (!('valueTypeCurrent' in state)) component.root._beforecreate.push(select_change_handler);
			...
			m: function mount(target, anchor) {
				...
				selectOption(select, state.valueTypeCurrent);
			...
			p: function update(changed, state) {
				...
				if (!select_updating) selectOption(select, state.valueTypeCurrent);
	...
	function create_if_block_12(state, component) {
		...
		return {
			...
			p: function update(changed, state) {
				if (!input_updating) input.value = state.wholeEntry[2];
				if (changed.classInput) {
					input.className = state.classInput;
				}
