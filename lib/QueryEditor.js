(function (global, factory) {
	...
	function create_each_block(state, remainingOptions_1, option_1, option_index, component) {
		var option,
		    text_value = option_1,
		    text,
		    option_value_value;
		...
		return {
			...
			h: function hydrate() {
				option.__value = option_value_value = option_1;
			...
			p: function update(changed, state, remainingOptions_1, option_1, option_index) {
				if (changed.remainingOptions && text_value !== (text_value = option_1)) {
					text.data = text_value;
				}

				if (changed.remainingOptions && option_value_value !== (option_value_value = option_1)) {
	...
	function create_if_block(state, component) {
		...
		function select_change_handler() {
			...
			component.set({ __newOption: selectValue(select) });
			select_updating = false;
		}
		...
		return {
			c: function create() {
				...
				option = createElement("option");
				text_1 = createText("- Select an Option -");
			...
			h: function hydrate() {
				...
				if (!('__newOption' in state)) component.root._beforecreate.push(select_change_handler);
			...
			m: function mount(target, anchor) {
				...
				selectOption(select, state.__newOption);
			},
			...
			p: function update(changed, state) {
				...
				if (!select_updating) selectOption(select, state.__newOption);
	...
	function create_each_block_1(state, optionsKeysTypes_1, option, option_index, component) {
		var div,
		    div_1,
		    text_value = option.key,
		    text,
		    text_1,
		    text_2,
		    div_2,
		...
		var current_block_type = select_block_type(state, optionsKeysTypes_1, option, option_index);
		var if_block = current_block_type(state, optionsKeysTypes_1, option, option_index, component);
		...
		return {
			...
			p: function update(changed, state, optionsKeysTypes_1, option, option_index) {
				if (changed.optionsKeysTypes && text_value !== (text_value = option.key)) {
					text.data = text_value;
				}

				if (current_block_type === (current_block_type = select_block_type(state, optionsKeysTypes_1, option, option_index)) && if_block) {
					if_block.p(changed, state, optionsKeysTypes_1, option, option_index);
				} else {
					if_block.u();
					if_block.d();
					if_block = current_block_type(state, optionsKeysTypes_1, option, option_index, component);
					if_block.c();
					if_block.m(div_2, null);
				}
	...
	function create_each_block_2(state, optionsKeysTypes_1, option, option_index, supportedClasses, curClass, curClass_index, component) {
		var option_1,
		...
		return {
			...
			h: function hydrate() {
				option_1.__value = option_1_value_value = curClass.class;
			...
			p: function update(changed, state, optionsKeysTypes_1, option, option_index, supportedClasses, curClass, curClass_index) {
				if (changed.supportedClasses && text_value !== (text_value = curClass.class)) {
					text.data = text_value;
				}

				if (changed.supportedClasses && option_1_value_value !== (option_1_value_value = curClass.class)) {
	...
	function create_each_block_3(state, optionsKeysTypes_1, option, option_index, type, enumVal, enumVal_index, component) {
		var option_1,
		    text_value = enumVal,
		    text,
		    option_1_value_value;
		...
		return {
			...
			h: function hydrate() {
				option_1.__value = option_1_value_value = enumVal;
				option_1.value = option_1.__value;
			},
			...
			p: function update(changed, state, optionsKeysTypes_1, option, option_index, type, enumVal, enumVal_index) {
				if (changed.optionsKeysTypes && text_value !== (text_value = enumVal)) {
					text.data = text_value;
				}

				if (changed.optionsKeysTypes && option_1_value_value !== (option_1_value_value = enumVal)) {
					option_1.__value = option_1_value_value;
				}
	...
	function create_if_block_1(state, optionsKeysTypes_1, option, option_index, component) {
		var select,
		    select_updating = false;
		...
		for (var i = 0; i < supportedClasses.length; i += 1) {
			each_blocks[i] = create_each_block_2(state, optionsKeysTypes_1, option, option_index, supportedClasses, supportedClasses[i], i, component);
		}

		function select_change_handler() {
			var state = component.get();
			select_updating = true;
			state.options[option.key] = selectValue(select);
			component.set({ options: state.options });
			select_updating = false;
		}
		...
		return {
			...
			h: function hydrate() {
				addListener(select, "change", select_change_handler);
				if (!('options' in state)) component.root._beforecreate.push(select_change_handler);
				select.className = state.classSelect;
			...
			m: function mount(target, anchor) {
				...
				selectOption(select, state.options[option.key]);
			},
			...
			p: function update(changed, state, optionsKeysTypes_1, option, option_index) {
				...
				if (changed.supportedClasses) {
					for (var i = 0; i < supportedClasses.length; i += 1) {
						if (each_blocks[i]) {
							each_blocks[i].p(changed, state, optionsKeysTypes_1, option, option_index, supportedClasses, supportedClasses[i], i);
						} else {
							each_blocks[i] = create_each_block_2(state, optionsKeysTypes_1, option, option_index, supportedClasses, supportedClasses[i], i, component);
							each_blocks[i].c();
							each_blocks[i].m(select, null);
				...
				if (!select_updating) selectOption(select, state.options[option.key]);
				if (changed.classSelect) {
					select.className = state.classSelect;
				}
	...
	// (35:99) 
	function create_if_block_2(state, optionsKeysTypes_1, option, option_index, component) {
		...
		if (option.key in state.options) {
			valueeditor_initial_data.value = state.options[option.key];
		...
		var valueeditor = new _ValueEditor2.default({
			...
			_bind: function _bind(changed, childState) {
				...
				if (!valueeditor_updating.value && changed.value) {
					state.options[option.key] = childState.value;
		...
		return {
			...
			p: function update(changed, state, optionsKeysTypes_1, option, option_index) {
				...
				if (changed.optionsKeysTypes) valueeditor_changes.valueTypeInitial = option.type;
				if (changed.optionsKeysTypes) valueeditor_changes.allowedTypes = [option.type];
				...
				if (!valueeditor_updating.value && changed.options || changed.optionsKeysTypes) {
					valueeditor_changes.value = state.options[option.key];
					valueeditor_updating.value = true;
				}
	...
	// (43:10) {{else}}
	function create_if_block_3(state, optionsKeysTypes_1, option, option_index, component) {
		var select,
		    select_updating = false;
		...
		for (var i = 0; i < type.length; i += 1) {
			each_blocks[i] = create_each_block_3(state, optionsKeysTypes_1, option, option_index, type, type[i], i, component);
		}
		...
		return {
			...
			p: function update(changed, state, optionsKeysTypes_1, option, option_index) {
				...
				if (changed.optionsKeysTypes) {
					for (var i = 0; i < type.length; i += 1) {
						if (each_blocks[i]) {
							each_blocks[i].p(changed, state, optionsKeysTypes_1, option, option_index, type, type[i], i);
						} else {
							each_blocks[i] = create_each_block_3(state, optionsKeysTypes_1, option, option_index, type, type[i], i, component);
							each_blocks[i].c();
							each_blocks[i].m(select, null);
	...
	function create_if_block_5(state, component) {
		var div, div_1, text, div_2, h3, text_2, div_3, text_11, div_4, text_13, div_5, text_20, div_6, text_40, div_7, text_42, div_8, text_55, div_9, button;
		...
		return {
			...
			m: function mount(target, anchor) {
				...
				appendNode(text_40, div_2);
				appendNode(div_7, div_2);
				appendNode(text_42, div_2);
				appendNode(div_8, div_2);
				appendNode(text_55, div_2);
	...
	function select_block_type(state, optionsKeysTypes_1, option, option_index) {
		if (option.type === "class") return create_if_block_1;
		if (option.type === "int" || option.type === "boolean" || option.type === "string") return create_if_block_2;
	...
	function click_handler(event) {
		...
		var optionsKeysTypes_1 = this._svelte.optionsKeysTypes_1,
		    ...
		    option = optionsKeysTypes_1[option_index];
		component.removeOption(option.key);
	}
	...
	function QueryEditor(options) {
		...
		if (!document.getElementById("svelte-3932320288-style")) add_css();
